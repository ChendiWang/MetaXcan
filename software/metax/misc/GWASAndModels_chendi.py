...
Edited by Chendi on 2019-01-11
"""
...
def align_data_to_alleles(data, base, left_on, right_on):
    ...
    logging.log(9, "%s rows of merged data after deleting strand switch", merged.shape[0])
    """
    ...
    # logging.log(9, "%s rows in alleles_1_2", alleles_1_2.shape[0])
    # logging.log(9, "alleles1_2 ENtry 0: %s", alleles_1_2[0])
    # logging.log(9, "alleles1_2 Entry 1: %s", alleles_1_2[1])
    ...
    logging.log(9, "%s rows to be switched", sum(switched))

    logging.log(9, "EA Entry to be switched: %s", merged.loc[switched, EA])
    merged.loc[switched, EA] = strand_switch(merged.loc[switched, EA])
    logging.log(9, "EA Entry after being switched: %s", merged.loc[switched, EA])

    logging.log(9, "NEA Entry to be switched: %s", merged.loc[switched, NEA])
    merged.loc[switched, NEA] = strand_switch(merged.loc[switched, NEA])
    logging.log(9, "NEA Entry after being switched: %s", merged.loc[switched, NEA])
    ...
    logging.log(9, "%s rows to be deleted due to A&T case", sum(delete_case_AT))
    delete_case_CG = alleles_1_2 == set([u'C', u'G'])
    logging.log(9, "%s rows to be deleted due to C&G case", sum(delete_case_CG)) """
    ...
    logging.log(9, "%s rows to be DELETED", sum(keep_case==False))
    logging.log(9, "%s rows to be KEPT", sum(keep_case))

    merged = merged[keep_case]
    logging.log(9, "%s rows of merged data after dropping the discarded cases", merged.shape[0])   

    # =============== Handle allele flip =============== 
    flipped = merged[EA] != merged[EA_BASE]

    logging.log(9, "%s rows to be flipped", sum(flipped))
    # logging.log(9, "%s Bases to be flipped", flipped)
    ...
    logging.log(9, "%s rows before final checking", merged.shape[0])
    merged = merged[keep_case_final]
    logging.log(9, "%s rows after final checking", merged.shape[0])
